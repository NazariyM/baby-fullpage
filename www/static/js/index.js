...
var _detectIE = __webpack_require__(8);
...
var _objectFitVideos = __webpack_require__(16);

var _objectFitVideos2 = _interopRequireDefault(_objectFitVideos);

var _objectFitImages = __webpack_require__(17);
...
var _initSliders = __webpack_require__(19);

var _initAccordion = __webpack_require__(21);

var _contactTabs = __webpack_require__(22);

var _contactTabs2 = _interopRequireDefault(_contactTabs);

var _ChooseMap = __webpack_require__(23);

var _initYouTube = __webpack_require__(24);

var _initParticles = __webpack_require__(25);

var _initContactBlock = __webpack_require__(27);

var _vintagePopup = __webpack_require__(28);

var _vintagePopup2 = _interopRequireDefault(_vintagePopup);

var _cTabs = __webpack_require__(29);
...
var Common = exports.Common = function () {
  ...
  _createClass(Common, null, [{
    key: 'init',
    value: function init() {
      (0, _initPreload.initPreload)();
      new _Fullpage2.default();
      new _Header2.default();
      (0, _detectIE.getIEVersion)();
      (0, _objectFitImages2.default)();
      (0, _initSliders.initSliders)();
      (0, _initParticles.initParticles)();
      (0, _initAccordion.initAccordion)();
      (0, _ChooseMap.initChooseMap)();
...
var Fullpage = function () {
  ...
  _createClass(Fullpage, [{
    ...
    value: function fullpage() {
      ...
      function initScreenVideo() {
        ...
        if ($screenBlock.length) {
          var $video = $('.screen__video').find('video')[0];
      ...
      function initAnimation(nextSection) {
        var nextElements = nextSection.find('[data-anim]');
        var fastElem = nextSection.find('[data-anim-fast]');
        var slowElem = nextSection.find('[data-anim-slow]');

        var tl = new _gsap.TimelineMax({ delay: 0.25 });
        var tlSlow = new _gsap.TimelineMax({ delay: 0.5 });

        tl.staggerFromTo(nextElements, 0.8, { y: 80, opacity: 0 }, { y: 0, opacity: 1 }, 0.5, '-=0.5');

        tl.staggerFromTo(fastElem, 0.5, { y: 100, opacity: 0 }, { y: 0, opacity: 1 }, 0.3, '-=0.1');

        tlSlow.staggerFromTo(slowElem, 1, { y: 120, opacity: 0 }, { y: 0, opacity: 1 }, 0.6);
      }

      function titleAnimation(el) {
        var title = el.find('[data-anim-title]');

        var timeLine = new TimelineLite({ delay: 0.2 }),
            splitText = new _SplitText2.default(title, { type: 'words,chars' }),
      ...
      // move to top
      $('.scroll-up a').on('click tap', function () {
    ...
    value: function removeVideo() {
      if (_helpers.Resp.isMobile) $('.screen__video').remove();
    }
  }, {
    key: 'init',
    value: function init() {
...
var Header = function () {
  ...
  _createClass(Header, [{
    ...
    value: function initMobNav() {
      ...
      var $navLink = _helpers.$nav.find('.nav__list-link');
      ...
      $navLink.on('click tap', function () {
        _helpers.$nav.removeClass(_helpers.css.active);
        $btn.removeClass(_helpers.css.active);
      });
    }
  }, {
    key: "init",
...
function initSliders() {
  ...
  var $interiorSld = $('.js-interior-slider');
  ...
  var $expertSld = $('.js-expert-slider');
  ...
  var $librarySld = $('.js-library-slider');
...
function initAccordion() {
  ...
  $accordion.each(function () {
    ...
    var $btn = $item.find('> div:first-child');
    var $content = $item.find('.accordion__content');
    var speed = 0.5;

    $item.each(function () {
      var $thisContent = $(this).find('.accordion__content');
    ...
    $btn.on('click tap', function () {
      var $selfItem = $(this).closest($item);
      var $selfContent = $selfItem.find($content);

      $item.not($selfItem).removeClass(_helpers.css.active);
      $content.each(function () {
        if (!$(this).closest($selfItem).length) {
          hideContent($(this));
        }
      });
      $selfItem.toggleClass(_helpers.css.active);

      if ($selfItem.hasClass(_helpers.css.active)) {
    ...
    function showContent(el) {
      _gsap.TweenMax.set(el, { height: 'auto', scaleY: 1 });
      _gsap.TweenMax.from(el, speed, { height: 0, scaleY: 0 });
    }

    function hideContent(el) {
      _gsap.TweenMax.to(el, speed, { height: 0, scaleY: 0 });
    }
  });
};
...
var ContactTabs = function () {
  function ContactTabs(el) {
    _classCallCheck(this, ContactTabs);
  ...
  _createClass(ContactTabs, [{
    ...
    value: function bindEvents() {
      ...
      this.$tabNav.on('click', function (ev) {
        var currentIndex = _this2.getActiveIndex();
    ...
    value: function changeTab(currentIndex, nextIndex) {
      ...
      var $nextTab = this.$tabItem.eq(nextIndex);

      $currentTabNav.removeClass(_helpers.css.active);
      ...
      _gsap.TweenMax.to($currentTab, speed, {
        ...
        onComplete: function onComplete() {
          ...
          _gsap.TweenMax.set(_this.$tabItemContainer, { height: currentHeight });
          $(this.target).hide();
          _gsap.TweenMax.set($nextTab, { autoAlpha: 1 });
          $nextTab.show();
          _gsap.TweenMax.staggerFromTo($nextTab.children(), speed, {
            autoAlpha: 0,
            y: -10
          }, {
            autoAlpha: 1,
...
var $tabs = $('.contact__nav');
$tabs.each(function (index, el) {
  var tab = new ContactTabs($(el));
  tab.init();
});
...
exports.initChooseMap = initChooseMap;
function initChooseMap() {
  ...
  function init() {
    var moscowCenter = [55.75366848567328, 37.621676554687504];
    var mapMoscow = new ymaps.Map('choose-map', {
      center: moscowCenter,
      zoom: 9,
      controls: []
    });
...
function initYouTube() {
  ...
  $playFrame.on('click tap', function (e) {
    e.preventDefault();
    var $this = $(this);
    var $videoText = $this.siblings('.js-video-text');
    var frame = this.nextElementSibling;

    $this.fadeOut();
    $this.siblings('iframe').fadeIn(speed, function () {
      play(frame);
    });

    $videoText.addClass('is-float');

    $tabsBtn.on('click tap', function () {
      stop(frame);
    });
  });
...
var _particles = __webpack_require__(26);
...
exports.initContactBlock = initContactBlock;
...
function initContactBlock() {
  ...
  function init() {
    var moscowCenter = [55.75366848567328, 37.621676554687504];
    var contactMapMoscow = new ymaps.Map('contact-map-moscow', {
      center: moscowCenter,
      zoom: 10,
      controls: []
    }),
    ...
    function showClubBranch() {
      var $branchList = $('.js-branch-list');
      var $branchBtn = $branchList.children();

      $branchBtn.on('click', function (e) {
        e.preventDefault();
        var $that = $(this);
        var $branchNumber = $that.index() + 1;
    ...
    var $clubLink = $('.js-club-link');

    $clubLink.on('click', function (e) {
      e.preventDefault();
      var $that = $(this);
      var $clubCoords = $that.data('club-coords');
  ...
  function showInfo() {
    var $info = $('.js-contact-info');
    var $infoBtn = $info.find('.contact__info-title');
    var $infoDetails = $info.find('.contact__info-details');
    var duration = 150;
    ...
    $infoBtn.on('click', function (e) {
      ...
      $infoBtn.removeClass(_helpers.css.active);
      $that.addClass(_helpers.css.active);
      $infoDetails.slideUp(duration);
      $thatDetails.slideDown(duration);
    });

    $infoBtn.each(function () {
      $(this).hasClass(_helpers.css.active) ? $(this).trigger('click') : false;
    });
  }

  function chooseBranch() {
    var $branchList = $('.js-branch-list');
    var $branchBtn = $branchList.find('.contact__metro-branch');
    ...
    $branchBtn.on('click', function (ev) {
      ...
      $that.addClass(_helpers.css.active);
      $that.siblings().removeClass(_helpers.css.active);
    });
  }
}
